    ...
    public class POTranslationRepository : ITranslationRepository
    {
        ...
        public void SaveTranslation(Translation translation)
        {
            ...
            using (StreamWriter stream = new StreamWriter(filePath))
            {
                ...
                stream.WriteLine("\"POT-Creation-Date: \\n\"");
                stream.WriteLine("\"PO-Revision-Date: \\n\"");
                stream.WriteLine("\"Language-Team: " + translation.LanguageInformation.LanguageInEnglish + "\\n\"");
                stream.WriteLine("\"Language: " + translation.LanguageInformation.LanguageShortTag + "\\n\"");
                stream.WriteLine("\"Plural-Forms: " + PluralsDictionary.Get(translation.LanguageInformation.LanguageShortTag) + "\\n\"");
                ...
                foreach (var item in orderedItems)
                {
                    ...
                    if (String.IsNullOrEmpty(item.MsgIdPlural))
                    {
                        WriteString(stream, hasReferences, "msgstr", escape((item.Messages ?? new List<String> { "" }).First()));
                    }
                    else
                    {
                        WriteString(stream, hasReferences, "msgid_plural", escape(item.MsgIdPlural));
                        for (int count = 0; count < nplurals; count++)
                        {
                            WriteString(stream, hasReferences, "msgstr[" + count + "]",
                                escape((item.Messages ?? new String[nplurals]).ElementAt(count)));
                        }
                    }
        ...
        public void SaveTemplate(IDictionary<string, TemplateItem> items)
        {
            ...
            using (StreamWriter stream = new StreamWriter(filePath))
            {
                ...
                lang.LanguageShortTag = _settings.InitialLanguage;
                //This is required for poedit to read the files correctly if they contains for instance swedish characters
                stream.WriteLine("msgid \"\"");
                stream.WriteLine("msgstr \"\"");
                stream.WriteLine("\"Project-Id-Version: \\n\"");
                stream.WriteLine("\"POT-Creation-Date: \\n\"");
                stream.WriteLine("\"Language-Team: " + lang.LanguageInEnglish + "\\n\"");
                stream.WriteLine("\"Language: " + lang.LanguageShortTag + "\\n\"");
                stream.WriteLine("\"Plural-Forms: " + PluralsDictionary.Get(lang.LanguageShortTag) + "\\n\"");
                stream.WriteLine("\"MIME-Version: 1.0\\n\"");
                stream.WriteLine("\"Content-Type: text/plain; charset=utf-8\\n\"");
                stream.WriteLine("\"Content-Transfer-Encoding: 8bit\\n\"");
                stream.WriteLine("\"X-Generator: i18n.POTGenerator\\n\"");
                ...
                var nplurals = 1;
                int.TryParse(PluralsDictionary.Get(lang.LanguageShortTag).Split(';')[0].Split('=')[1], out nplurals);
                foreach (var item in orderedItems)
                {
                    ...
                    // enable loading of POT file into editor e.g. PoEdit.
                    if (String.IsNullOrEmpty(item.MsgIdPlural))
                    {
                        WriteString(stream, true, "msgstr", "");
                    }
                    else
                    {
                        WriteString(stream, true, "msgid_plural", escape(item.MsgIdPlural));
                        for (int count = 0; count < nplurals; count++)
                        {
                            WriteString(stream, true, "msgstr[" + count + "]", "");
                        }
                    }
            ...
            if (File.Exists(path))
            {
                ...
                using (var fs = File.OpenText(path))
                {
                    ...
                    while ((line = fs.ReadLine()) != null)
                    {
                        ...
                        {
                            TranslationItem item = ParseBody(fs, line, extractedComments, langtag);
                            if (item != null)
                            {
                                //
        ...
        private TranslationItem ParseBody(TextReader fs, string line, List<string> extractedComments, string langtag)
        {
            ...
            if (line.StartsWith("msgid"))
            {
                ...
                while ((line = fs.ReadLine()) != null)
                {
                    ...
                    if ((!line.StartsWith("msgstr") && !line.StartsWith("msgid_plural")) && (msgid = Unquote(line)) != null)
                    {
                        sb.Append(msgid);
                    }
                    else
            ...
            if (line.StartsWith("msgid_plural"))
            {
                ...
                while ((line = fs.ReadLine()) != null)
                {
                    ...
                    if (!line.StartsWith("msgstr") && (msgid_plural = Unquote(line)) != null)
                    {
                        sb.Append(msgid_plural);
                    }
                    else
                ...
                message.MsgIdPlural = Unescape(sb.ToString());
            }
            ...
            {
                var nplurals = 1;
                int.TryParse(PluralsDictionary.Get(langtag).Split(';')[0].Split('=')[1], out nplurals);
                var messages = new string[nplurals];
                var msgidn = int.Parse(line.Substring(7, 1));
                ...
                while ((line = fs.ReadLine()) != null && (msgstr = Unquote(line)) != null)
                {
                    ...
                    {
                        messages[msgidn] = Unescape(sb.ToString());
                        sb.Clear();
                        msgidn = int.Parse(line.Substring(7, 1));
                        msgstr = Unquote(line);
                        sb.Append(msgstr);
                    }
                    else
            ...
            else if (!string.IsNullOrEmpty(line) && line.StartsWith("msgstr"))
            {
                var msgstr = Unquote(line);
                sb.Append(msgstr);
                ...
                message.Messages = new List<string> { Unescape(sb.ToString()) };
            }
            return message;
        }
